<?php
  $title_align_class = $single_page_title_align = $values = $titlebar_bg = $shop_titlebar_bg = $shop_title_bar_text_align = $blog_title_switch = $enable_topbar_mobile = $title_bar_style = '';
  ...
  if ( '' != ekko_get_option( 'tek-blog-header-text-align' ) ) {
    $title_align_class = ekko_get_option( 'tek-blog-header-text-align' );
  }
  ...
  $blog_title_switch = ekko_get_option( 'tek-blog-title-switch' );
...
<?php elseif ( class_exists( 'Tribe__Events__Main' ) && ( ( tribe_is_event() || tribe_is_event_category() || tribe_is_in_main_loop() || tribe_is_view() || is_post_type_archive( 'tribe_events' ) || 'tribe_events' == get_post_type() || is_singular( 'tribe_events' ) ) ) ) : ?>
   <header class="entry-header blog-header events-calendar-header <?php echo esc_attr( $enable_topbar_mobile ); ?> <?php if ( ekko_get_option( 'tek-topbar' ) == '1') { echo esc_attr( 'with-topbar' ); } ?>">
   </header>

<?php elseif (class_exists( 'WooCommerce' ) && is_woocommerce()) : ?>
  <?php if (!(ekko_get_option( 'tek-woo-single-header' ) == '0' && is_product())) : ?>
    <header class="entry-header blog-header <?php echo esc_attr( $enable_topbar_mobile ); ?> <?php if ( ekko_get_option( 'tek-topbar' ) == '1') { echo esc_attr( 'with-topbar' ); } ?>" <?php if ( $shop_titlebar_bg != '' ) { echo 'style="background-color: '.$shop_titlebar_bg.';"'; } ?>>
...
<?php elseif(is_home() || is_search() || ( is_archive() && !is_tax( 'portfolio-category' ) ) || is_single() && !is_singular( 'portfolio' )) : ?>
  <header class="entry-header blog-header <?php echo esc_attr( $enable_topbar_mobile ); ?> <?php if ( ekko_get_option( 'tek-topbar' ) == '1') { echo esc_attr( 'with-topbar' ); } ?>">
     <div class="row blog-page-heading <?php echo esc_attr($title_align_class); ?>">
        <?php if (!is_single()) : ?>
          <?php if ( ekko_get_option( 'tek-blog-header-template' ) == 'blog-header-titlebar' && is_array( $keydesign_header_image ) ) : ?>
        ...
        <div class="container">
          ...
          <?php elseif ( is_search() ) : ?>
            <h1 class="section-heading">
               <?php apply_filters( 'kd_search_query_title', esc_html_e("Search results for:", "ekko") ); ?> <?php the_search_query();  ?>
            </h1>
          <?php elseif ( is_category() ) : ?>
            <h1 class="section-heading">
               <?php apply_filters( 'kd_single_cat_title', esc_html_e("Currently browsing:", "ekko") ); ?> <?php single_cat_title(); ?>
            </h1>
          <?php elseif ( is_tag() ) : ?>
            <h1 class="section-heading">
               <?php apply_filters( 'kd_single_tag_title', esc_html_e("All posts tagged:", "ekko") ); ?> <?php single_tag_title(); ?>
            </h1>
          <?php elseif ( is_author() ) : ?>
            <h1 class="section-heading">
               <?php apply_filters( 'kd_author_archive_title', esc_html_e("All posts by", "ekko") ); ?> <?php echo esc_html(get_userdata(get_query_var('author'))->display_name); ?>
            </h1>
          <?php elseif ( is_day() ) : ?>
            <h1 class="section-heading">
               <?php apply_filters( 'kd_day_archive_title', esc_html_e("Posts archive for", "ekko") ); ?> <?php echo get_the_date('F jS, Y'); ?>
            </h1>
          <?php elseif ( is_month() ) : ?>
            <h1 class="section-heading">
               <?php apply_filters( 'kd_month_archive_title', esc_html_e("Posts archive for", "ekko") ); ?> <?php echo get_the_date('F, Y'); ?>
            </h1>
          <?php elseif ( is_year() ) : ?>
            <h1 class="section-heading">
              <?php apply_filters( 'kd_year_archive_title', esc_html_e("Posts archive for", "ekko") ); ?> <?php echo get_the_date('Y'); ?>
            </h1>
          ...
          <?php if ( '' != ekko_get_option( 'tek-blog-subtitle' ) && is_home() ) : ?>
            <p class="section-subheading"><?php echo esc_html( ekko_get_option( 'tek-blog-subtitle' ) ); ?></p>
          <?php endif; ?>
          ...
          <?php endif; ?>
          <?php if( is_home() ) : ?>
            <?php if ( ekko_get_option( 'tek-blog-home-breadcrumbs' ) != '0' ) : ?>
              <?php if( function_exists('bcn_display') ) : ?>
                <div class="breadcrumbs" typeof="BreadcrumbList" vocab="https://schema.org/">
                  <?php bcn_display(); ?>
                </div>
          ...
          <?php if ( ekko_get_option( 'tek-blog-header-form' ) && is_home()) : ?>
            <?php if( !empty( ekko_get_option( 'tek-blog-header-form-id' ) ) && ekko_get_option( 'tek-blog-header-form' ) == 1 ) : ?>
              <div class="blog-header-form">
                <?php echo do_shortcode('[contact-form-7 id="'. esc_attr( ekko_get_option( 'tek-blog-header-form-id' ) ).'"]'); ?>
              </div>
            <?php endif; ?>
          <?php endif; ?>
        </div>
...
<?php elseif( is_tax( 'portfolio-category' ) ) : ?>
  <header class="entry-header single-page-header portfolio-category-header <?php echo esc_attr( $enable_topbar_mobile ); ?> <?php if ( ekko_get_option( 'tek-topbar' ) == '1') { echo esc_attr( 'with-topbar' ); } ?>">
    <div class="row single-page-heading blog-title-center">
        <div class="container">
          <h1 class="section-heading">
            <?php echo apply_filters( 'kd_portfolio_cat_title', esc_html__("Currently browsing:", "ekko") ); ?> <?php single_cat_title(); ?>
          </h1>
          <?php if( function_exists('bcn_display') ) : ?>
            <div class="breadcrumbs" typeof="BreadcrumbList" vocab="https://schema.org/">
              <?php bcn_display(); ?>
...
<?php elseif( !is_404() && !is_singular( 'portfolio' ) ) : ?>
  <?php if ( empty($themetek_page_showhide_title_section) && !is_single()) : ?>
    <header class="entry-header single-page-header <?php echo esc_attr( $enable_topbar_mobile ); ?> <?php if ( ekko_get_option( 'tek-topbar' ) == '1') { echo esc_attr( 'with-topbar' ); } ?>" <?php if ( $title_bar_style != '' ) { echo 'style="' . $title_bar_style . '"'; } ?>>

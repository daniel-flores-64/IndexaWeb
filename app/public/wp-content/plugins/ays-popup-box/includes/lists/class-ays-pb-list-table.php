...
//                    $url = esc_url_raw( remove_query_arg(false) ) . 'ays_pb_tab='.$ays_pb_tab."&status=" . $message . "&type=success";
    ...
    function column_title( $item ) {
        ...
        $restitle  = Ays_Pb_Admin::ays_pb_restriction_string("word",esc_attr($item["title"]), $popup_title_length);

        $title = sprintf( "<a href='?page=%s&action=%s&popupbox=%d' title='%d'>%s</a>", esc_attr( $_REQUEST["page"] ), "edit", absint( $item["id"] ), esc_attr($item["title"]), $restitle);
